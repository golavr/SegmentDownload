...
using System.Net;
using SegmentDownloader.Core.Concurrency;
        ...
        private void SegmentThreadProc(object objSegment)
        {
            ...
            try
            {
                ...
                using (segment.InputStream)
                {
                    ...
                    // Fix from code project:
                    // http://www.codeproject.com/Messages/4730070/A-bug-fix.aspx
                    if (segment.State == SegmentState.Downloading
                       && segment.StartPosition != segment.EndPosition)
                    {
                        Debug.WriteLine("Probably stream read has been timeout.");
